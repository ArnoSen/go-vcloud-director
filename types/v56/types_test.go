...
`
	...
	if myAdminVDC.ResourcePoolRefs == nil {
		t.Fatal("Did not find any resource pool refs but was expecting 1 result")
	}
	if len(myAdminVDC.ResourcePoolRefs.VimObjectRef) != 1 {
		t.FailNow()
	}
	if myAdminVDC.ResourcePoolRefs.VimObjectRef[0].MoRef != "resgroup-1696" {
		t.FailNow()
	}
	if myAdminVDC.ResourcePoolRefs.VimObjectRef[0].VimObjectType != "RESOURCE_POOL" {
		t.FailNow()
	}
	if myAdminVDC.ResourcePoolRefs.VimObjectRef[0].VimServerRef == nil {
		t.FailNow()
	}
	...
	if *myAdminVDC.ResourcePoolRefs.VimObjectRef[0].VimServerRef != expectedVIMServerRef {
		t.FailNow()
	}
}
